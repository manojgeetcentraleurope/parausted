-- ============================================================
-- Migration: harden confirm_stripe_purchase_and_issue_voucher
-- Fixes transaction safety for transient failures.
--
-- Root cause addressed:
--   In the previous version, voucher code generation exhaustion used
--   RETURN jsonb_build_object('success', false, 'error', 'unknown').
--   A plain RETURN is not an exception — it does NOT roll back DML.
--   The processed_webhooks insert and purchase UPDATE from earlier in
--   the same transaction would be committed, leaving a confirmed
--   purchase with no voucher. Stripe retries would then be silently
--   swallowed by the already_processed guard.
--
-- Changes:
--   1. Voucher code exhaustion now raises RAISE EXCEPTION, causing
--      the whole transaction (including processed_webhooks) to roll
--      back. Stripe can retry successfully.
--   2. Removed EXCEPTION WHEN OTHERS THEN RETURN block. Unexpected
--      DB errors now propagate naturally, rolling back the transaction.
--      The caller receives a DB error, the route returns HTTP 500,
--      and Stripe retries.
--   3. Added explicit IF NOT FOUND guard after gift card lookup.
--      Missing gift card is a permanent data integrity failure;
--      processed_webhooks is committed and retrying will not help.
--
-- Permanent errors (deliberate RETURN — processed_webhooks committed,
-- Stripe retries correctly suppressed):
--   invalid_input, unsupported_event_type, not_found,
--   invalid_payment_source, already_processed, expired
--
-- Transient errors (RAISE EXCEPTION — transaction rolls back,
-- Stripe retries will reprocess the event):
--   voucher_code_generation_failed, any unexpected DB exception
-- ============================================================
