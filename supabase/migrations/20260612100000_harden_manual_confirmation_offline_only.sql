-- Harden manual confirmation so only OFFLINE purchases can be confirmed
-- through the merchant-facing voucher issuance RPC, and revoke execute
-- access from the deprecated confirm-only RPC.
...
REVOKE ALL ON FUNCTION public.confirm_pending_purchase(UUID) FROM anon;
REVOKE ALL ON FUNCTION public.confirm_pending_purchase(UUID) FROM authenticated;
