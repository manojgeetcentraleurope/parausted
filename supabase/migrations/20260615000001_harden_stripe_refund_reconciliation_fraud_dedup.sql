-- ============================================================
-- Slice 8b.6e: Harden public.reconcile_stripe_refund_webhook
-- ParaUsted - Digital Gift Card SaaS
--
-- Tiny DB hardening of the external Stripe refund reconciliation RPC:
--
--   1. De-duplicate open fraud_flags for the same economic refund.
--      Stripe emits multiple events (refund.created, refund.updated)
--      for one succeeded refund_id, which previously produced duplicate
--      open fraud_flags rows. Before every fraud_flags insert we now
--      skip insertion when an OPEN flag already exists for the same
--      rule_code + refund_id (scoped by purchase_id, or purchase_id IS
--      NULL for pre-mapping/unmapped conflicts). Outcomes are unchanged.
--
--   2. Quieten canceled/failed refund events that arrive AFTER an
--      existing conflict flag. If the refund is terminal (failed or
--      canceled), the purchase is NOT refund_pending, and an open
--      fraud_flags row already exists for the same purchase_id +
--      refund_id, we audit-only (conflict = refund_terminal_after_
--      existing_flag) and return the new stable outcome
--      'conflict_refund_terminal_after_existing_flag' instead of
--      raising another external_refund_status_mismatch flag.
--
-- Everything else is unchanged: SECURITY DEFINER, search_path,
-- service_role-only grants, processed_webhooks idempotency, no
-- EXCEPTION WHEN OTHERS, no Stripe/network call, no redemption
-- mutation, no ledger/payout/delivery/email side effects, and all
-- existing successful reconciliation + conflict outcomes.
-- ============================================================
...
BEGIN
    ...
    -- --------------------------------------------------------
    -- 2b. Serialize concurrent processing for the SAME refund_id.
    --     Stripe can deliver refund.created / refund.updated /
    --     refund.failed for one refund nearly concurrently. Without
    --     this lock, two transactions could both pass the IF NOT EXISTS
    --     fraud_flags dedup checks and insert duplicate open flags.
    --     This transaction-scoped advisory lock is keyed on p_refund_id
    --     only, so it serializes events for the same refund without
    --     globally locking all refund reconciliations. Released
    --     automatically at transaction end.
    -- --------------------------------------------------------
    PERFORM pg_advisory_xact_lock(
        hashtext('stripe_refund_reconciliation'),
        hashtext(p_refund_id)
    );
    ...
    IF v_refund_status NOT IN ('succeeded', 'failed', 'canceled') THEN
        -- Dedup: skip when an open flag already exists for this rule + refund.
        IF NOT EXISTS (
            SELECT 1 FROM fraud_flags
            WHERE purchase_id IS NULL
              AND rule_code = 'external_refund_unknown_status'
              AND status = 'open'
              AND evidence->>'refund_id' = p_refund_id
        ) THEN
    ...
    IF p_purchase_id IS NOT NULL THEN
        ...
        IF NOT FOUND THEN
            ...
            IF NOT EXISTS (
                SELECT 1 FROM fraud_flags
                WHERE purchase_id IS NULL
                  AND rule_code = 'external_refund_unmapped'
                  AND status = 'open'
                  AND evidence->>'refund_id' = p_refund_id
            ) THEN
                INSERT INTO fraud_flags (
    ...
    ELSIF p_payment_intent_id IS NOT NULL AND trim(p_payment_intent_id) <> '' THEN
        ...
        IF (
            ...
            IF NOT EXISTS (
                SELECT 1 FROM fraud_flags
                WHERE purchase_id IS NULL
                  AND rule_code = 'external_refund_mapping_ambiguous'
                  AND status = 'open'
                  AND evidence->>'refund_id' = p_refund_id
            ) THEN
                INSERT INTO fraud_flags (
    ...
    IF v_currency IS NOT NULL AND lower(v_currency) <> lower(v_purchase.currency) THEN
        ...
        IF NOT EXISTS (
            ...
            WHERE purchase_id = v_purchase.id
              AND rule_code = 'external_refund_currency_mismatch'
              AND status = 'open'
              AND evidence->>'refund_id' = p_refund_id
        ) THEN
            INSERT INTO fraud_flags (
    ...
    IF NOT FOUND THEN
        ...
        IF NOT EXISTS (
            SELECT 1 FROM fraud_flags
            WHERE purchase_id = v_purchase.id
              AND rule_code = 'external_refund_missing_voucher'
              AND status = 'open'
              AND evidence->>'refund_id' = p_refund_id
        ) THEN
            INSERT INTO fraud_flags (
    ...
    IF v_purchase.status = 'refunded' THEN
        ...
        IF NOT EXISTS (
            SELECT 1 FROM fraud_flags
            WHERE purchase_id = v_purchase.id
              AND rule_code = 'external_refund_id_mismatch'
              AND status = 'open'
              AND evidence->>'refund_id' = p_refund_id
        ) THEN
            INSERT INTO fraud_flags (
    ...
    IF v_refund_status = 'succeeded' THEN
        ...
        IF p_refund_amount_cents < v_purchase.amount_cents THEN
            ...
            IF NOT EXISTS (
                SELECT 1 FROM fraud_flags
                WHERE purchase_id = v_purchase.id
                  AND rule_code = 'external_partial_refund_detected'
                  AND status = 'open'
                  AND evidence->>'refund_id' = p_refund_id
            ) THEN
                INSERT INTO fraud_flags (
        ...
        IF p_refund_amount_cents > v_purchase.amount_cents THEN
            ...
            IF NOT EXISTS (
                SELECT 1 FROM fraud_flags
                WHERE purchase_id = v_purchase.id
                  AND rule_code = 'external_refund_amount_mismatch'
                  AND status = 'open'
                  AND evidence->>'refund_id' = p_refund_id
            ) THEN
                INSERT INTO fraud_flags (
    ...
    IF v_refund_status = 'succeeded' AND v_redemption_count > 0 THEN
        ...
        IF NOT EXISTS (
            SELECT 1 FROM fraud_flags
            WHERE purchase_id = v_purchase.id
              AND rule_code = 'external_refund_after_redemption'
              AND status = 'open'
              AND evidence->>'refund_id' = p_refund_id
        ) THEN
            INSERT INTO fraud_flags (
    ...
    -- --------------------------------------------------------
    -- 15b. Terminal refund (failed/canceled) arriving AFTER an existing
    --      open fraud flag for the same economic refund. This commonly
    --      happens when a merchant cancels in Stripe a refund that we
    --      already flagged as a conflict (e.g. refund after redemption).
    --      Avoid emitting another noisy external_refund_status_mismatch
    --      flag. Audit-only; do not mutate purchase/voucher; do not
    --      auto-clear the existing flag.
    -- --------------------------------------------------------
    IF v_refund_status IN ('failed', 'canceled')
       AND v_purchase.status <> 'refund_pending'
       AND EXISTS (
           SELECT 1 FROM fraud_flags
           WHERE purchase_id = v_purchase.id
             AND status = 'open'
        ...
        INSERT INTO audit_events (
            ...
            v_audit_payload || jsonb_build_object(
                ...
                'voucher_status',   v_voucher.status,
                'redemption_count', v_redemption_count,
                'conflict',         'refund_terminal_after_existing_flag'
            )
        );
    ...
    IF NOT EXISTS (
        SELECT 1 FROM fraud_flags
        WHERE purchase_id = v_purchase.id
          AND rule_code = 'external_refund_status_mismatch'
          AND status = 'open'
          AND evidence->>'refund_id' = p_refund_id
    ) THEN
        INSERT INTO fraud_flags (
...
-- ============================================================
-- Grants: service_role only (re-asserted; unchanged from baseline)
-- ============================================================
...
IS 'DB-only reconciliation for external Stripe refund webhook events (refund.created, refund.updated, refund.failed). Handles idempotency via processed_webhooks, maps purchase by explicit id or payment_intent_id, and deterministically transitions purchase/voucher state or emits fraud_flags. De-duplicates open fraud_flags per rule_code + refund_id (scoped by purchase_id, or NULL pre-mapping). Terminal failed/canceled events arriving after an existing open flag for the same refund are audited only (conflict_refund_terminal_after_existing_flag) without raising a duplicate status_mismatch flag. No Stripe calls, no ledger/payout/delivery/email side effects, no redemption mutation. p_charge_id is evidence-only and not used for purchase mapping. GRANT EXECUTE to service_role only.';
